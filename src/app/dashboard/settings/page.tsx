...
export default function SettingsPage() {
  ...
  const fetchSettings = useCallback(async (uid: string) => {
    try {
      ...
      setMessage({ type: "error", text: "Exception loading settings." });
    }
  }, []);
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      fetchSettings(session.user.id).then(() => setLoading(false));
    });
  }, [fetchSettings]);
  ...
  const handleSaveModel = async () => {
    ...
    showMessage(ok ? "success" : "error", ok ? "Model saved." : "Failed to save model.");
  };
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-4 rounded-lg bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-sm font-medium text-gray-900">OpenRouter Model</h3>
            <p className="mt-1 text-xs text-gray-500">Choose which LLM model to use via OpenRouter</p>
          </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          <button
            ...
            disabled={savingModel || settings.openrouter_model === savedSettings.openrouter_model}
            className="rounded-md bg-blue-600 px-4 py-1.5 text-xs font-medium text-white hover:bg-blue-700 disabled:opacity-50"
          >
            {savingModel ? "Saving..." : "Save Model"}
          </button>
          {savedSettings.openrouter_model && settings.openrouter_model !== savedSettings.openrouter_model && (
            <button
              onClick={() => setSettings((prev) => ({ ...prev, openrouter_model: savedSettings.openrouter_model }))}
      ...
      <div className="space-y-4">
        {API_KEYS.map((apiKey) => {
          ...
          return (
            <div key={apiKey.id} className="rounded-lg bg-white p-6 shadow-sm">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-sm font-medium text-gray-900">{apiKey.label}</h3>
                  <p className="mt-1 text-xs text-gray-500">{apiKey.description}</p>
                </div>
                <span
                  className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                    hasSavedKey ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                  }`}
                >
              ...
              {!isEditing && hasSavedKey && (
                <div className="mt-4">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => setVisibility((prev) => ({ ...prev, [apiKey.field]: !prev[apiKey.field] }))}
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={() => setEditing((prev) => ({ ...prev, [apiKey.field]: true }))}
                    ...
                    {confirmDelete === apiKey.field ? (
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {deletingField === apiKey.field ? "Deleting..." : "Yes, delete"}
                        </button>
                        <button
              ...
              {(isEditing || !hasSavedKey) && (
                <div className="mt-4">
                  <input
                    type="text"
                    value={value}
                    onChange={(e) => setSettings((prev) => ({ ...prev, [apiKey.field]: e.target.value }))}
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      disabled={!value.trim() || savingField === apiKey.field}
                      className="rounded-md bg-blue-600 px-4 py-1.5 text-xs font-medium text-white hover:bg-blue-700 disabled:opacity-50"
                    >
                      {savingField === apiKey.field ? "Saving..." : hasSavedKey ? "Update" : "Save"}
                    </button>
                    {hasSavedKey && (
                      <button
